...
public class FischerEvent {
    private HikariDataSource hikari;
    private Essentials essentials = Job.getInstance().essentials;
    public FischerEvent(HikariDataSource hikari){
        this.hikari = hikari;
    }
    ...
    public void inect(Player player) {
        ...
        if (!JobManager.getPlayerjob(playername).equalsIgnoreCase("Fischer")) {
            return;
        }
        int needXp = (JobManager.getPlayerjoblevel(playername) * 9) + 10;
        ...
        if (xp >= needXp) {
            ...
            player.sendMessage("§7Du bekommst nun §b+0.5 §7mehr fürs Fischen.");
            player.sendMessage(" ");
            player.giveExp(10*JobManager.getPlayerjoblevel(playername));


            new BossBarManager().setProgress(0.0, player);
            new BossBarManager().setBarTitle("§b§lFischer §blevel §e" + JobManager.getPlayerjoblevel(playername), player);
            JobSql jobSql = new JobSql(hikari);
            jobSql.setPlayerJobLevel(player.getUniqueId().toString(), "fischer", JobManager.getPlayerjoblevel(playername));
            return;
        ...
        JobManager.setPlayerjobxp(playername, xp + 2);
